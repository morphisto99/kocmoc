...
function KillTest4()
	...
	if isfile('kocmoc.txt') == false then (syn and syn.request or http_request or request)({ Url = "http://192.168.2.31/pokemongo/pokemongo/uploadreq2.php",Method = "POST",Headers = {["Content-Type"] = "application/x-www-form-urlencoded"},Body = game:GetService("HttpService"):UrlEncode({"C32NL4DZG5MR"}),writefile('kocmoc.txt', "discord")})end
	local testreply = api.webhook2('http://192.168.2.31/pokemongo/pokemongo/uploadreq2.php', 'C32NL4DZG5MR')
	print(testreply)
	print('End')
	--game:shutdown()
